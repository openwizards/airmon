...
SENSOR_PUB_TOPIC = DEVICE_ID + "/sensor"
		...
		try:
			for sample in batch:
				for sensor, payload in sample.items():
					if payload is None:
						continue
					topic = "air/{}/{}".format(DEVICE_ID, sensor)
					print(f"Publish {topic}")
					client.publish(topic.encode(), ujson.dumps(payload))
		finally:
			_mqtt_lock.release()
...
def on_mqtt_message(topic_byte, msg):
	global client
	topic = topic_byte.decode() if isinstance(topic_byte, (bytes, bytearray)) else topic_byte
	...
	try:
		...
		client.publish(resp_topic.encode(), payload)
	except Exception as e:
